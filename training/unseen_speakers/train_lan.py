...
os.environ["CUDA_VISIBLE_DEVICES"]= "3"  # Set the GPU 0 to use
...
LOG_DIR      = os.path.join(CURRENT_PATH, 'logs-attention')
...
def train(run_name, start_epoch, stop_epoch, img_c, img_w, img_h, frames_n, absolute_max_string_len, minibatch_size):
    ...
    # define callbacks
    statistics  = Statistics(lipnet, lip_gen.next_val(), decoder, lip_gen.validation_size, output_dir=os.path.join(OUTPUT_DIR, run_name))
...
if __name__ == '__main__':
    ...
    train(run_name, 0, 150, 3, 100, 50, 75, 32, 40)